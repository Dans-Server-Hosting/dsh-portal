...
const LOOK: Record<ServerState, { label: string; color: "default" | "info" | "warning" | "success" | "error"; hint: string }> = {
  provisioning: { label: "provisioning", color: "info", hint: "Being set up; usually about a minute" },
  ...
  stopped: { label: "stopped", color: "default", hint: "The game was stopped from the dashboard or crashed; Wake starts it again" },
...
export default function StatePill({ state, size = "medium" }: { state: ServerState; size?: "small" | "medium" }) {
  ...
  return (
    <Chip
      ...
      color={look.color}
      size={size}
      variant={state === "asleep" || state === "stopped" ? "outlined" : "filled"}
      title={look.hint}
